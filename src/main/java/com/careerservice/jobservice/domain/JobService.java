...
public class JobService {
    ...
    public Job viewJobDetails (String jobid){
        return jobRepository.findByJobid(jobid).orElseThrow(() -> new JobNotFoundException(jobid));
    }
    ...
    public Job editJobDetails(String jobid, Job job){
        return jobRepository.findByJobid(jobid).map(exsistingJob -> {
            var jobToUpdate = new Job(exsistingJob.jobid(), job.title(), job.description(),
                    job.companyname(), job.skill1(), job.skill2());
